<%@page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<jsp:include page="header/header-full.jsp" />
<jsp:include page="header/nav.jsp">
	<jsp:param name="activeMenuIndex" value="3" />
	<jsp:param name="displaySearch" value="true" />
	<jsp:param name="subtitle"
		value="Contributors to the Cambridge Digital Library" />
</jsp:include>


<div class="campl-row campl-content campl-recessed-content">
	<div class="campl-wrap clearfix">

		<!-- side nav -->
		<div class="campl-column3">
			<div class="campl-tertiary-navigation">
				<div class="campl-tertiary-navigation-structure">
					<ul class="campl-unstyled-list campl-vertical-breadcrumb">
						<li><a href="/">Cambridge Digital Library<span
								class="campl-vertical-breadcrumb-indicator"></span></a></li>
					</ul>
					<ul
						class="campl-unstyled-list campl-vertical-breadcrumb-navigation">
						<li class="campl-selected"><a href="/about/">About</a>
							<ul
								class='campl-unstyled-list campl-vertical-breadcrumb-children'>
								<li><a href="/about/">Introducing the Cambridge Digital
										Library</a></li>
								<li><a href="/news/">News</a></li>
								<li><a href="/contributors/">Contributors</a></li>
								<li><a href="/terms/">Terms & Conditions</a></li>
							</ul></li>

					</ul>
				</div>
			</div>
		</div>


		<div class="campl-column8  campl-main-content" id="content">
			<div class="campl-content-container">
				<h2>Funders</h2>
				<hr class="cam-teaser-divider campl-column12">

				<div class="campl-column12 funder_item">
					<div class="campl-column3">
						<img src="/images/PolonskySmall.gif" title="Polonsky Foundation" />
					</div>
					<div class="campl-column8">
						Under the leadership of Dr Leonard Polonsky and as part of its
						International Digitisation Project, The <b>Polonsky Foundation</b>
						has provided major funding towards the first phase of the
						Cambridge Digital Library (2010-14), enabling the development of
						the digital library's core infrastructure and the digitisation of
						content for its Foundation Faith and Science collections.
					</div>
				</div>

				<div class="campl-column12 funder_item">

					<div class="campl-column3">
						<img src="/images/jisc.gif" title="Jisc" />
					</div>
					<div class="campl-column8">
						<b>Jisc</b> awarded a grant to Cambridge University Library and
						the Newton Project at the University of Sussex for the <a
							href="http://www.jisc.ac.uk/whatwedo/programmes/digitisation/econtent11/windows.aspx"
							target="_blank">Windows on Genius</a> project (<a
							href="http://www.jisc.ac.uk/fundingopportunities/funding_calls/2010/09/grant1110.aspx"
							target="_blank">Jisc e-Content Programme 2011</a>). This project
						enabled the linking of Newton's facsimiles and transcriptions.
						Jisc also funded the <a target="_blank"
							href="/collections/longitude"> Board of Longitude Collection</a>
						(<a target="_blank"
							href="http://www.jisc.ac.uk/whatwedo/programmes/digitisation/content2011_2013.aspx">JISC
							Content Programme 2011-2013</a>) a collaboration with the National
						Maritime Museum and Department of History and Philosophy of
						Science at Cambridge. This project created a substantial
						collection related to 18th century science and technology.
					</div>
				</div>

				<div class="campl-column12 funder_item">
					<div class="campl-column3">&nbsp;</div>
					<div class="campl-column8">
						The digitisation of the Taylor-Schechter Cairo Genizah Collection
						has been sponsored by the <b><a
							href="http://www.jewishmanuscripts.org/" target="_blank">Jewish
								Manuscript Preservation Society</a></b>, the <b><a
							href="http://www.genizah.org/" target="_blank">Friedberg
								Genizah Project Inc.</a></b>, and the <b><a
							href="http://www.ahrc.ac.uk/" target="_blank">Arts and
								Humanities Research Council, UK</a></b>.
					</div>
				</div>

				<div class="campl-column12 funder_item">
					<div class="campl-column3">
						<img src="/images/collectionsView/ahrc-logo.jpg" title="AHRC" />
					</div>
					<div class="campl-column8">
						The cataloguing and digitisation of the <a
							href="/collections/sanskrit">Sanskrit Collection</a> and <a
							href="/collections/spanishchapbooks">Spanish Chapbooks</a> were
						funded by the <b><a href="http://www.ahrc.ac.uk/"
							target="_blank">Arts and Humanities Research Council</a></b>, UK.

					</div>
				</div>

				<div class="campl-column12 funder_item">
					<div class="campl-column3">
						<img src="/images/collectionsView/nsf-logo.gif"
							title="Polonsky Foundation" />
					</div>
					<div class="campl-column8">
						The digitisation of the Darwin-Hooker letters has been funded
						through a grant to the Darwin Correspondence Project from the <b><a
							href="http://www.nsf.gov/" target="_blank">National Science
								Foundation</a></b>.
					</div>

				</div>

				<div class="campl-column12 funder_item">

					<div class="campl-column3">
						<img src="/images/neh_logo.jpg" title="NEH" />
					</div>
					<div class="campl-column8">
						The <a href="http://www.neh.gov/">National Endowment for the
							Humanities</a> (NEH) is an independent US federal agency created by
						Congress in 1965 to promote "excellence in the humanities". It is
						one of the largest funders of humanities programs in the United
						States. NEH supported the digitisation of all items in the Charles
						Darwin's Evolution Manuscripts under grant no. PW-51320-13.
					</div>

				</div>


				<div class="campl-column12 funder_item">

					<div class="campl-column3">
						<img src="/images/hamf_logo.jpg" title="HAMF" />
					</div>
					<div class="campl-column8">
						Much of the content in the Exhibitions collection has been made
						possible by a generous gift from the <a
							href="http://www.howardandabbymilsteinfoundation.org/">Howard
							and Abby Milstein Foundation</a>.
					</div>

				</div>


				<div class="campl-column12 funder_item">

					<div class="campl-column3">&nbsp;</div>
					<div class="campl-column8">Materials for the Japanese Works
						collections were digitised through a generous donation by
						Professor Mikiko Ishii of Kanagawa University, Japan.</div>

				</div>
				
				<div class="campl-column12 funder_item">

					<div class="campl-column3">&nbsp;</div>
					<div class="campl-column8">Digital imaging of the Royal Commonwealth Society collection generously supported by library grants from the <a href="http://www.ist.admin.cam.ac.uk/smuts-memorial-fund">Smuts Memorial Fund</a></div>

				</div>

				<hr class="cam-teaser-divider campl-column12">

				<h2>Digital Library Leadership</h2>

				<ul>
					<li>Dr Ben Outhwaite, Acting Joint Head of Special
						Collections, Senior Programme Lead</li>
					<li>Dr Christopher Stokoe, Head of Digital Library Development</li>
					<li>Grant Young, Head of Digital Content</li>

				</ul>

				<h2>Digital Library Development Department</h2>

				<ul>
					<li>Hal Blackburn, Senior Software Developer</li>
					<li>Jennie Fletcher, Senior Software Developer</li>
					<li>Dr Christopher Stokoe, Head of Digital Library Development</li>
					<li>Dr Lei Zhang, Developer</li>

				</ul>

				<h3>Department Alumni</h3>

				<ul>
					<li>Tony Stevenson, Operations</li>
					<li>Gilleain Torrance, Senior Software Developer</li>
				</ul>


				<h2>Digital Content Unit</h2>

				<ul>
					<li>Mark Box, Digitisation Technician</li>
					<li>Nancy Buck, Digitisation Assistant</li>
					<li>Annie Casciano, Digitisation Assistant</li>
					<li>Andy Corrigan, Digital Content Assistant</li>
					<li>Amelie Deblauwe, Digitisation Assistant</li>
					<li>Huw Jones, Digital Content Specialist</li>
					<li>Scott Maloney, Digitisation Assistant</li>
					<li>Blazej Mikula, Digitisation Technician</li>
					<li>Maciej Pawlikowski, Senior Photographer and Digitisation
						Supervisor</li>
					<li>Mark Scudder, Digitisation Technician</li>
					<li>Grant Young, Head of Digital Content</li>
				</ul>


				<h3>Department Alumni</h3>

				<ul>
					<li>Virginia Apuzo, Digitisation Assisant</li>
					<li>Les Goody, Senior Photographer</li>
					<li>Pete Johnston, Metadata Specialist</li>
					<li>Rachel Johnson, Photographer</li>
					<li>Don Manning, Head of Imaging Services</li>
					<li>Josh Murfitt, Digitisation Assistant</li>
					<li>Antonio Novena, Digitisation Assistant</li>
				</ul>


				<h2>Conservation Team</h2>

				<ul>
					<li>James Bloxam, Head of Conservation</li>
					<li>Lucy Cheng, Conservator</li>
					<li>Rebecca De'ath, Conservator</li>
					<li>Deborah Farndell, Conservator</li>
					<li>Mary French, Conservator</li>
					<li>Rebecca Goldie, Conservator</li>
					<li>Fay Humphries, Conservator</li>
					<li>Anna Johnson, Conservator</li>
					<li>Emma Nichols, Conservator</li>
					<li>Ngaio Vince-Dewerse, Conservator</li>
				</ul>



				<h2>Board of Longitude Collection</h2>
				<ul>
					<li>Professor Simon Schaffer, Professor of the History and
						Philosophy of Science, University of Cambridge</li>
					<li>Lucinda Blaser, Digital Project Manager, Royal Museums
						Greenwich</li>
					<li>Dr Richard Dunn, Senior Curator and Head of Science and
						Technology, Royal Museums Greenwich</li>
				</ul>

				<h2>Cairo Genizah Collection</h2>
				<ul>
					<li>Dr Ben Outhwaite, Head of the Genizah Research Unit, <a
						target="_blank"
						href="http://www.lib.cam.ac.uk/Taylor-Schechter/Unit.html">
							members of the Genizah Research Unit team</a> and <a target="_blank"
						href="http://www.lib.cam.ac.uk/Taylor-Schechter/contributors.html">other
							contributors</a>.
					</li>
				</ul>

				<h2>Christian Works</h2>
				<ul>
					<li>Dr Patrick Zutshi, Keeper of Manuscripts and University
						Archives</li>
					<li>Ed Potten, Head of Rare Books</li>
					<li>Jayne Ringrose, Cambridge University Library</li>

				</ul>


				<h2>Darwin-Hooker Collection</h2>
				<ul>
					<li>Professor Jim Secord, Director, Darwin Correspondence
						Project</li>
					<li>Dr Alison Pearn, Associate Director, Darwin Correspondence
						Project</li>
				</ul>


				<h2>Darwin Manuscripts</h2>
				<ul>
					<li>Professor David Kohn, Director, Darwin Manuscripts Project</li>
					<li>Dr Alison Pearn, Associate Director, Darwin Correspondence
						Project</li>
					<li>Professor Jim Secord, Director, Darwin Correspondence
						Project</li>

				</ul>

				<h2>Hebrew Manuscripts</h2>
				<ul>
					<li>Catherine Ansorge, Former Joint-Head of Near and Middle
						Eastern Department, Cambridge University Library</li>
					<li>Dr Ben Outhwaite, Head of the Genizah Research Unit</li>

				</ul>

				<h2>Islamic Manuscripts</h2>

				<ul>
					<li>Yasmin Faghihi, Joint-Head of Near and Middle Eastern
						Department</li>
					<li>Catherine Ansorge, Former Joint-Head of Near and Middle
						Eastern Department</li>
					<li>Dr Ignacio Sanchez, Specialist Cataloguer</li>
				</ul>


				<h2>Michaelides Fragments</h2>

				<ul>
					<li>Yasmin Faghihi, Joint-Head of Near and Middle Eastern
						Department</li>
					<li>Gennady Kurin, Specialist Cataloguer</li>
				</ul>

				<h2>Japanese Works</h2>
				<ul>
					<li>Noboru Koyama, Japanese Department, Cambridge University
						Library</li>

				</ul>
				
				<h2>Chinese Works</h2>
				<ul>
					<li>Charles Aylmer, Chinese Department, Cambridge University Library</li>
				</ul>


				<h2>Music</h2>
				<ul>
					<li>Dr Suzanne Paul, Manuscripts Department, Cambridge
						University Library</li>
					<li>Anna Pensaert, Head of Music Collections, Cambridge
						University Library and Pendlebury Library of Music</li>
					<li>Professor John Robinson, Lute Society</li>

				</ul>


				<h2>Newton Papers Collection</h2>

				<ul>
					<li>Adam Perkins, Curator of Scientific Manuscripts</li>
					<li>Rob Iliffe, Editorial Director of the Newton Project and
						Professor of Intellectual History and History of Science,
						University of Sussex and <a target="_blank"
						href="http://www.newtonproject.sussex.ac.uk/prism.php?id=24">other
							members of the Newton Project team</a> for the transcriptions.
					</li>

				</ul>


				<h2>Peterhouse Manuscripts</h2>
				<ul>
					<li>Scott Mandelbrote, Director of Studies in History, and
						Perne Librarian, Peterhouse</li>
					<li>Professor Kari Anne Rand, Department of Literature, Area
						Studies and European Languages, University of Oslo</li>
					<li>Seb Falk, Department of History and Philosophy of Science,
						University of Cambridge</li>
				</ul>


				<h2>Sanskrit Manuscripts</h2>
				<ul>
					<li>Dr Vincenzo Vergiani, Principal Investigator and Lecturer
						in Sanskrit, University of Cambridge</li>
					<li>Dr Daniele Cuneo, Research Associate</li>
					<li>Dr Camillo Formigatti, Research Associate</li>
				</ul>

				<h2>Sassoon Journals</h2>
				<ul>
					<li>Dr Emma Saunders, Archivist, Cambridge University Library</li>
					<li>John Wells, Curator of Literary Archives, Cambridge
						University Library</li>
				</ul>

				<h2>Spanish Chapbooks</h2>
				<ul>
					<li>Professor Alison Sinclair, Principal Investigator and
						Professor of Modern Spanish Literature and Intellectual History,
						University of Cambridge</li>
					<li>Sonia Morcillo-García, Hispanic Specialist Cataloguer</li>
				</ul>

				<h2>Waterloo</h2>
				<ul>
					<li>Dr Mark Nicholls, St John's College, Cambridge</li>
					<li>John Wells, Curator of Literary Archives, Cambridge
						University Library</li>
				</ul>
				
				

				<h2>Treasures of the Library Collection</h2>
				<ul>
					<li>Dr Patrick Zutshi, Keeper of Manuscripts and University
						Archives</li>
					<li>Ed Potten, Head of Rare Books</li>
					<li>Professor Paul Binski, Professor of the History of
						Medieval Art, University of Cambridge</li>
					<li>Professor David Parker, Edward Cadbury Professor of
						Theology and Director of the Institute for Textual Scholarship and
						Electronic Editing, University of Birmingham</li>
				</ul>



				<h2>Contact Details</h2>
				<p>
					Cambridge Digital Library<br /> Cambridge University Library<br />
					West Road<br /> Cambridge<br /> United Kingdom<br /> CB3 9DR<br />
					Email: <a href="mailto:dl-feedback@lib.cam.ac.uk">dl-feedback@lib.cam.ac.uk</a><br />
				</p>


			</div>
		</div>
	</div>
</div>


<jsp:include page="header/footer-full.jsp" />

